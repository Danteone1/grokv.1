...
Laboratorio SAF + Sociofísica + 5 modos de datos:
  real | dummy | synth_coherent | synth_pure | synth_calib

+ Adversario + Bounded Confidence + SPOF + Optimizador + 77 Preguntas
+ Visualización + Influencia dirigida (E0–E5/SIM)
+ CONFIG externa + BaseModel intercambiable + Experiment.run/validate/save
...
CONTIGUIDAD_ALCALDIAS = {
    "CUAUHTEMOC": ["MIGUEL HIDALGO", "BENITO JUAREZ", "VENUSTIANO CARRANZA", "AZCAPOTZALCO"],
    "BENITO JUAREZ": ["CUAUHTEMOC", "COYOACAN", "IZTACALCO", "MIGUEL HIDALGO"],
    "IZTAPALAPA": ["IZTACALCO", "TLAHUAC", "XOCHIMILCO", "COYOACAN", "VENUSTIANO CARRANZA"],
    "GUSTAVO A MADERO": ["AZCAPOTZALCO", "VENUSTIANO CARRANZA", "CUAUHTEMOC"],
    "COYOACAN": ["BENITO JUAREZ", "TLALPAN", "XOCHIMILCO", "IZTAPALAPA", "ALVARO OBREGON"],
    "TLALPAN": ["COYOACAN", "XOCHIMILCO", "LA MAGDALENA CONTRERAS", "ALVARO OBREGON"],
    "XOCHIMILCO": ["TLALPAN", "COYOACAN", "IZTAPALAPA", "TLAHUAC", "MILPA ALTA"],
    "ALVARO OBREGON": ["COYOACAN", "TLALPAN", "LA MAGDALENA CONTRERAS", "CUAJIMALPA DE MORELOS", "MIGUEL HIDALGO"],
    "MIGUEL HIDALGO": ["CUAUHTEMOC", "BENITO JUAREZ", "ALVARO OBREGON", "AZCAPOTZALCO"],
    "VENUSTIANO CARRANZA": ["CUAUHTEMOC", "IZTAPALAPA", "IZTACALCO", "GUSTAVO A MADERO"],
    "AZCAPOTZALCO": ["CUAUHTEMOC", "MIGUEL HIDALGO", "GUSTAVO A MADERO"],
    "IZTACALCO": ["BENITO JUAREZ", "IZTAPALAPA", "VENUSTIANO CARRANZA"],
    "TLAHUAC": ["IZTAPALAPA", "XOCHIMILCO", "MILPA ALTA"],
    "MILPA ALTA": ["XOCHIMILCO", "TLAHUAC"],
...
class DataProvider:
    """Única puerta de entrada de datos. Devuelve (df_state, adj, meta)."""

    def __init__(self, mode: str = "synth_pure", seed: int = 42, cfg: dict | None = None):
        self.mode = mode
        self.seed = seed
    ...
    def _load_real(self, secciones_csv=None, shp_path=None, electoral_csv=None,
                   socio_csv=None, seguridad_csv=None, p_intra=0.08, p_inter=0.02):
        if secciones_csv is None:
            raise ValueError("Modo real requiere al menos secciones_csv")
        df = pd.read_csv(secciones_csv) if not hasattr(secciones_csv, "read") else pd.read_csv(secciones_csv)
        ...
        if "lat" not in df.columns or "lon" not in df.columns:
            if shp_path is not None:
                try:
                    ...
                    gdf["seccion"] = gdf.get("seccion", gdf.index).astype(str).str.zfill(4)
                    centroids = gdf.geometry.centroid
                    gdf["lat"] = centroids.y
                    gdf["lon"] = centroids.x
                    df = df.merge(gdf[["seccion", "lat", "lon"]], on="seccion", how="left")
                except Exception:
                    df["lat"] = df["alcaldia"].map(lambda a: ALCALDIA_COORDS.get(a, (19.43, -99.13))[0])
                    df["lon"] = df["alcaldia"].map(lambda a: ALCALDIA_COORDS.get(a, (19.43, -99.13))[1])
            else:
        ...
        if electoral_csv is not None:
            elec = pd.read_csv(electoral_csv) if not hasattr(electoral_csv, "read") else pd.read_csv(electoral_csv)
            ...
            if "partido_o_coalicion" in elec.columns and "votos" in elec.columns:
                piv = elec.pivot_table(index="seccion", columns="partido_o_coalicion",
        ...
        df["spin"] = np.where(df["opinion_continua"] > 0.25, 1,
                              np.where(df["opinion_continua"] < -0.25, -1, 0))
        df["intencion"] = np.where(df["spin"] == 1, "SIMPATIZANTE",
        ...
        if seguridad_csv is not None:
            seg = pd.read_csv(seguridad_csv) if not hasattr(seguridad_csv, "read") else pd.read_csv(seguridad_csv)
        ...
        meta = {"mode": "real", "source_hash": sha256(str(df[["seccion", "alcaldia"]].head(20).values.tolist())),
    ...
    def _load_dummy(self, n_secciones: int = 48):
        alcaldias = ALCALDIAS_CDMX[:6]
        rows = []
        for i in range(n_secciones):
            alc = alcaldias[i % len(alcaldias)]
            lat, lon = ALCALDIA_COORDS[alc]
            rows.append({
                "seccion": f"{i+1:04d}", "alcaldia": alc,
                "lat": lat + self.rng.normal(0, 0.01), "lon": lon + self.rng.normal(0, 0.01),
                "opinion_continua": float(self.rng.uniform(-0.6, 0.6)),
                "capital_social": 0.55, "desconfianza": 0.35, "nivel_movilizacion": 0.50,
            })
        df = pd.DataFrame(rows)
        df["territorial_unit_id"] = df["seccion"]
        df["agent_id"] = "DUM-" + df["seccion"]
        df["spin"] = np.where(df["opinion_continua"] > 0.25, 1, np.where(df["opinion_continua"] < -0.25, -1, 0))
        df["intencion"] = np.where(df["spin"] == 1, "SIMPATIZANTE", np.where(df["spin"] == -1, "OPOSITOR", "INDECISO"))
        df = self._fill_required_columns(df)
        adj = self._build_adj_from_alcaldias(df, p_intra=0.15, p_inter=0.04)
        df["grado"] = df.index.map(lambda i: len(adj.get(i, [])))
        max_g = max(int(df["grado"].max() or 1), 1)
        df["influencia_SAF"] = (df["habilidades_sociales"] * 0.6 + (df["grado"] / max_g) * 0.4).clip(0, 1)
        meta = {"mode": "dummy", "source_hash": sha256(f"dummy-{n_secciones}-{self.seed}"),
                "n_nodes": len(df), "timestamp": pd.Timestamp.now().isoformat(),
                "notes": "Datos dummy mínimos para pruebas rápidas"}
        return df.reset_index(drop=True), dict(adj), meta

    def _load_synth_coherent(self, n: int = 400, stats: dict | None = None):
        if stats is None:
            stats = {"share_simpat_mean": 0.37, "share_simpat_std": 0.14,
                     "marginacion_mean": 0.42, "marginacion_std": 0.18,
                     "participacion_mean": 0.58, "participacion_std": 0.11}
        rows = []
        for i in range(n):
            alc = str(self.rng.choice(ALCALDIAS_CDMX))
            lat, lon = ALCALDIA_COORDS[alc]
            share = float(np.clip(self.rng.normal(stats["share_simpat_mean"], stats["share_simpat_std"]), 0.05, 0.95))
            opinion = 2 * share - 1
            margin = float(np.clip(self.rng.normal(stats["marginacion_mean"], stats["marginacion_std"]), 0.05, 0.95))
            part = float(np.clip(self.rng.normal(stats["participacion_mean"], stats["participacion_std"]), 0.2, 0.9))
            rows.append({
                "seccion": f"S{i:04d}", "alcaldia": alc,
                "lat": lat + self.rng.normal(0, 0.012), "lon": lon + self.rng.normal(0, 0.012),
                "opinion_continua": opinion,
                "capital_social": float(np.clip(1 - margin + self.rng.normal(0, 0.08), 0.1, 0.95)),
                "desconfianza": margin, "nivel_movilizacion": part,
            })
        df = pd.DataFrame(rows)
        df["territorial_unit_id"] = df["seccion"]
        ...
        max_g = max(int(df["grado"].max() or 1), 1)
        df["influencia_SAF"] = (df["habilidades_sociales"] * 0.6 + (df["grado"] / max_g) * 0.4).clip(0, 1)
        meta = {"mode": "synth_coherent", "source_hash": sha256(f"coherent-{n}-{self.seed}"),
                "n_nodes": len(df), "timestamp": pd.Timestamp.now().isoformat(),
                "notes": "Sintético coherente derivado de estadísticas realistas CDMX"}
        return df.reset_index(drop=True), dict(adj), meta
    ...
    def _load_synth_calib(self, scenario: str = "polarizacion_alta", n: int = 300, intensidad: float = 0.8):
        scenarios = {
        ...
        meta = {"mode": "synth_calib", "scenario": scenario, "intensidad": intensidad,
                "source_hash": sha256(f"calib-{scenario}-{n}-{intensidad}-{self.seed}"),
                "n_nodes": len(df), "timestamp": pd.Timestamp.now().isoformat(),
                "notes": f"Escenario de calibración: {scenario}"}
        return df, adj, meta
    ...
    def _scenario_polarizacion_alta(self, n=300, intensidad=0.8):
        ...
        for i in range(n):
            ...
            lat, lon = ALCALDIA_COORDS[alc]
            opinion = (0.7 + 0.25 * intensidad) if bloque == 0 else (-0.7 - 0.25 * intensidad)
            ...
            rows.append({"seccion": f"P{i:04d}", "alcaldia": alc,
                         ...
                         "opinion_continua": opinion, "capital_social": 0.6,
                         "desconfianza": 0.3 + 0.2 * intensidad, "nivel_movilizacion": 0.65})
        df = pd.DataFrame(rows)
    ...
    def _scenario_spof_critico(self, n=250, intensidad=0.85):
        ...
        for i in range(n):
            ...
            lat, lon = ALCALDIA_COORDS[alc]
            rows.append({"seccion": f"S{i:04d}", "alcaldia": alc,
                         "lat": lat + self.rng.normal(0, 0.01), "lon": lon + self.rng.normal(0, 0.01),
                         "opinion_continua": float(self.rng.uniform(-0.5, 0.5)),
                         "capital_social": 0.5, "desconfianza": 0.4, "nivel_movilizacion": 0.55})
        df = pd.DataFrame(rows)
        df = self._finalize_calib_df(df)
    ...
    def _scenario_cascade_rapida(self, n=280, intensidad=0.9):
        ...
        for i in range(n):
            ...
            rows.append({"seccion": f"C{i:04d}", "alcaldia": alc,
                         "lat": lat + self.rng.normal(0, 0.009), "lon": lon + self.rng.normal(0, 0.009),
                         "opinion_continua": opinion, "capital_social": 0.55,
                         "desconfianza": 0.25, "nivel_movilizacion": 0.7})
        df = pd.DataFrame(rows)
        df = self._finalize_calib_df(df)
    ...
    def _scenario_equilibrio_fragil(self, n=300, intensidad=0.7):
        ...
        for i in range(n):
            ...
            lat, lon = ALCALDIA_COORDS[alc]
            r = self.rng.random()
            if r < 0.34: opinion = 0.55
            elif r < 0.67: opinion = -0.55
            else: opinion = 0.0
            opinion = float(np.clip(opinion + self.rng.normal(0, 0.12), -1, 1))
            rows.append({"seccion": f"E{i:04d}", "alcaldia": alc,
                         "lat": lat + self.rng.normal(0, 0.01), "lon": lon + self.rng.normal(0, 0.01),
                         "opinion_continua": opinion, "capital_social": 0.5,
                         "desconfianza": 0.45, "nivel_movilizacion": 0.5})
        df = pd.DataFrame(rows)
        df = self._finalize_calib_df(df)
    ...
    def _scenario_broker_dominante(self, n=220, intensidad=0.9):
        ...
        lat, lon = ALCALDIA_COORDS[alc]
        nuevo = {
            "seccion": "BROKER01", "alcaldia": alc, "lat": lat, "lon": lon,
            "opinion_continua": 0.8, "capital_social": 0.95, "desconfianza": 0.1,
            "nivel_movilizacion": 0.9, "territorial_unit_id": "BROKER01",
            "agent_id": "BROKER-CALIB", "spin": 1, "intencion": "SIMPATIZANTE",
            "es_broker_insertado": True, "habilidades_sociales": 0.93,
            "fatiga": 0.0, "capital_politico": 0.95, "grado": 0, "influencia_SAF": 0.0,
            "acceso_informacion": 0.9, "influencia_liderazgo": 0.9, "arraigo": 0.8,
            "exposicion_problema": 0.3, "resistencia_institucional": 0.2,
            "temperatura_sintetica": 40.0, "polarizacion_local": 0.3, "calibrado": True,
        }
        df = pd.concat([df, pd.DataFrame([nuevo])], ignore_index=True)
        candidatos = list(range(hub_idx))
        sel = self.rng.choice(candidatos, size=min(35, len(candidatos)), replace=False)
        for s in sel:
            adj.setdefault(hub_idx, []).append(int(s))
            adj.setdefault(int(s), []).append(hub_idx)
        return self._finish_calib(df, adj)

    def _finalize_calib_df(self, df):
        df["territorial_unit_id"] = df["seccion"]
    ...
    def _fill_required_columns(self, df):
        defaults = {
            "capital_social": 0.5, "acceso_informacion": 0.5, "influencia_liderazgo": 0.4,
            "arraigo": 0.6, "nivel_movilizacion": 0.55, "desconfianza": 0.35,
            "exposicion_problema": 0.4, "resistencia_institucional": 0.3,
            "temperatura_sintetica": 55.0, "fatiga": 0.0, "capital_politico": 0.7,
            "es_broker_insertado": False, "es_adversario": False, "polarizacion_local": 0.4,
            "calibrado": False, "habilidades_sociales": 0.5,
        }
    ...
    def _build_adj_from_alcaldias(self, df, p_intra=0.08, p_inter=0.02):
...
if "s5" not in st.session_state:
    st.session_state.s5 = {"df": pd.DataFrame(), "adj": {}, "df_base": pd.DataFrame(), "adj_base": {},
                           ...
                           "config": CONFIG, "broker_ids": [], "adv_ids": [], "data_meta": {}}
...
data_mode = st.sidebar.selectbox(
    ...
    index=0,
    help="real=CSV+SHP | dummy=mínimo | coherent=derivado de stats | pure=sintético libre | calib=escenarios de prueba"
)
...
if data_mode == "real":
    ...
    seguridad_file = st.sidebar.file_uploader("Seguridad (CSV opcional)", type=["csv"])
    ...
    scenario = st.sidebar.selectbox(
        ...
        ["polarizacion_alta", "spof_critico", "cascade_rapida", "equilibrio_fragil", "broker_dominante"]
    )
    intensidad = st.sidebar.slider("Intensidad del escenario", 0.3, 1.0, 0.8, 0.05)
...
steps = st.sidebar.slider("Pasos ABM", 5, 40, 15)
n_sim = st.sidebar.slider("N sim Monte Carlo", 5, 40, 15)

with st.sidebar.expander("🧾 Config externa (YAML/JSON)"):
    st.caption("Parámetros fuera del código. Los sliders sobrescriben en runtime.")
    cfg_text = st.text_area("config", value=dump_config(S["config"]), height=180, key="cfg_text")
    if st.button("Aplicar config"):
        try:
            ...
            st.success("Config aplicada.")
        except Exception as e:
            st.error(f"Config inválida: {e}")

if st.sidebar.button("🧬 Cargar / Generar universo", type="primary"):
    provider = DataProvider(mode=data_mode, seed=int(seed), cfg=S["config"])
    try:
        if data_mode == "real":
            if secciones_file is None:
                ...
                df, adj, meta = provider.load(
                    ...
                    electoral_csv=electoral_file, socio_csv=socio_file,
                    seguridad_csv=seguridad_file, p_intra=p_intra, p_inter=p_inter)
                S.update({"df": df, "adj": adj, "df_base": df.copy(),
                          "adj_base": {k: list(v) for k, v in adj.items()},
                          "tray": pd.DataFrame(), "df_final": pd.DataFrame(),
                          "events": [], "n_events_total": 0, "mc": None,
                          "broker_ids": [], "adv_ids": [], "data_meta": meta})
                st.success(f"Universo real | nodos={meta['n_nodes']} | hash={meta['source_hash'][:12]}…")
        elif data_mode == "synth_calib":
            df, adj, meta = provider.load(scenario=scenario, n=n, intensidad=intensidad)
            S.update({"df": df, "adj": adj, "df_base": df.copy(),
                      "adj_base": {k: list(v) for k, v in adj.items()},
                      "tray": pd.DataFrame(), "df_final": pd.DataFrame(),
                      "events": [], "n_events_total": 0, "mc": None,
                      "broker_ids": [], "adv_ids": [], "data_meta": meta})
            st.success(f"Escenario {meta.get('scenario')} | nodos={meta['n_nodes']}")
            ...
            S.update({"df": df, "adj": adj, "df_base": df.copy(),
                      ...
                      "events": [], "n_events_total": 0, "mc": None,
                      "broker_ids": [], "adv_ids": [], "data_meta": meta})
            st.success(f"Dummy | nodos={meta['n_nodes']}")
            ...
            S.update({"df": df, "adj": adj, "df_base": df.copy(),
                      ...
                      "events": [], "n_events_total": 0, "mc": None,
                      "broker_ids": [], "adv_ids": [], "data_meta": meta})
            st.success(f"Coherent | nodos={meta['n_nodes']}")
        else:  # synth_pure
            df, adj, meta = provider.load(n=n, p_intra=p_intra, p_inter=p_inter)
            S.update({"df": df, "adj": adj, "df_base": df.copy(),
                      "adj_base": {k: list(v) for k, v in adj.items()},
                      "tray": pd.DataFrame(), "df_final": pd.DataFrame(),
                      "events": [], "n_events_total": 0, "mc": None,
                      "broker_ids": [], "adv_ids": [], "data_meta": meta})
            st.success(f"Sintético puro | nodos={meta['n_nodes']} | seed={seed}")
    except Exception as e:
        st.error(f"Error al cargar: {e}")

if st.sidebar.button("♻️ Restaurar universo base"):
    if "df_base" in S and not S["df_base"].empty:
        S["df"] = S["df_base"].copy()
        S["adj"] = {k: list(v) for k, v in S["adj_base"].items()}
        S.update({"tray": pd.DataFrame(), "df_final": pd.DataFrame(),
                  "events": [], "n_events_total": 0, "mc": None,
                  "broker_ids": [], "adv_ids": []})
